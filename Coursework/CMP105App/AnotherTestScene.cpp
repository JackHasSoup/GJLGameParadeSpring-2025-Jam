#include "AnotherTestScene.h"

AnotherTestScene::AnotherTestScene(sf::RenderTarget* hwnd) : Scene(hwnd)
{
...
void AnotherTestScene::update(float dt)
{
...
void AnotherTestScene::handleInput(float dt)
{
	cmndr.handleInput();
...
void AnotherTestScene::executeAndTrack(BufferedCommand* b)
{
	const int size = actionList.size();
	if (size < maxActions)
